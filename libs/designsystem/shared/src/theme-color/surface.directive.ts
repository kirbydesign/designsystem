 ...
 *
 * Designed to be reused as a host directive on any surface-like component (e.g. card, list).
 *
 * The theme is read from the sibling `ThemeColorDirective` on the *same* host element only
 * (`self: true`). Without `self`, the element-injector lookup would climb the DOM and resolve
 * an ancestor's `ThemeColorDirective`, causing nested surfaces to mirror their parent's
 * surface instead of establishing their own. When the host has no `themeColor`, the lookup
 * yields `null` and it falls back to a raised surface for the elevated/flat variants.
 */
@Directive({
  standalone: true,
})
