...
# Formatting Pipes
The following pipes are utilities that can be used to transform and format data for display purposes. They are particularly useful in localization, where data such as numbers, dates, and currencies need to be presented in a format that is familiar to the user based on their locale. The pipes are also useful for ensuring a common presentation of the supported data types across one or more applications.
...
<Description of={DateOnlyStories.DateOnly} />
<Canvas of={DateOnlyStories.DateOnly} />
<Description of={TimeOnlyStories.TimeOnly} />
<Canvas of={TimeOnlyStories.TimeOnly} />
<Description of={TimeOrDateStories.TimeOrDate} /> 
...
```ts
import { provideKirbyExtensionsToken } from '@kirbydesign/extensions-angular/localization';

providers: [
    provideKirbyExtensionsToken(() => ({
        /* @example 'DKK | kr. | EUR' */
        nativeCurrency: 'DKK',
        defaultLang: 'da',
        countryCode: '+45',
...
It is possible to experiment and interact with these by selecting the specific examples in the sidebar 👈
