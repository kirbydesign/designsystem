...
6. `git add libs/designsystem/icon/src/icons/svg/ libs/designsystem/icon/src/kirby-icon-settings.ts && git commit -m "feat(icon): Add icons — <names> (fixes #<NUMBER>)" && git push -u origin feat/add-icons-<NUMBER>`
7. Check for an existing open PR: `gh pr list --head feat/add-icons-<NUMBER> --state open --json number,url`. If found, report that URL. Otherwise `gh pr create --base develop --title "feat(icon): Add icons — <names>" --body "Closes #<NUMBER>" --assignee @me --label "feature"` and report the new URL.
