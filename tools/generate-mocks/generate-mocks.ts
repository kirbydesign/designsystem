...
`;
  ...
  private mergeExistingContent(
    mockClassName: string,
    autoGeneratedClassDeclaration: string,
    existingContent: string
  ) {
    const startClassDeclaration = `// start class ${mockClassName}`;
    const endClassDeclaration = `} // end class ${mockClassName}`;
    const startClassDeclarationStartIndex = existingContent.indexOf(startClassDeclaration);
    const endClassDeclarationStartIndex =
      existingContent.indexOf(endClassDeclaration) + endClassDeclaration.length;
    const existingClassDeclaration = existingContent.substring(
      startClassDeclarationStartIndex,
      endClassDeclarationStartIndex
    );

    const startCustomRegion = '  // #region CUSTOM';
    const endRegion = ' // #endregion';
    const customRegionStartIndex = existingClassDeclaration.indexOf(startCustomRegion);
    if (customRegionStartIndex > -1) {
      const customRegionEndIndex =
        existingClassDeclaration.indexOf(endRegion, customRegionStartIndex) + endRegion.length;
      const existingCustomContent = existingClassDeclaration.substring(
        customRegionStartIndex,
        customRegionEndIndex
      );

      const mergedClassDeclaration = `${autoGeneratedClassDeclaration.substring(
        0,
        autoGeneratedClassDeclaration.indexOf(endClassDeclaration)
      )}
${existingCustomContent}
${endClassDeclaration}
`;
      return mergedClassDeclaration;
    }
    return autoGeneratedClassDeclaration;
  }
  ...
  private async renderMock(fileName: string, newFilename: string, exportedComponents: string[]) {
    ...
    if (existsSync(newFilename)) {
      existingContent = readFileSync(newFilename).toString();
    }
    ...
    components.forEach((metaData) => {
      ...
      const autoGeneratedClassDeclaration = this.renderClass(
        metaData.className,
        mockClassName,
        metaData
      );
      const mergedClassDeclaration = this.mergeExistingContent(
        mockClassName,
        autoGeneratedClassDeclaration,
        existingContent
      );
      rendered.push(mergedClassDeclaration);
    });
...
// start class ${mockClassName}
export class ${mockClassName} {${propertiesString}} // end class ${mockClassName}
`;
    return content;
  }
