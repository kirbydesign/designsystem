...
`;
    ...
    for (const fileOrFolder of folderContent) {
      ...
      if (ent.isDirectory()) {
        ...
        if (fileOrFolder.endsWith('.component.ts')) {
          ...
          const classNames = this.renderMock(fullPath, newFilename, exportedComponents);
          if (classNames) {
  ...
  private renderMock(fileName: string, newFilename: string, exportedComponents: string[]) {
