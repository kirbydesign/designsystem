...
`;
  ...
  private async renderMock(fileName: string, newFilename: string) {
    ...
    let rxImportStatement = '';

    if (
      components.some((metaData) =>
        metaData.properties.some((prop) => prop.type && prop.type.indexOf('Observable'))
      )
    ) {
      rxImportStatement = `import { Observable } from 'rxjs';\n`;
    }
...
${endRegion}
`;
    let content = rxImportStatement + importStatement + autoGeneratedContent;
