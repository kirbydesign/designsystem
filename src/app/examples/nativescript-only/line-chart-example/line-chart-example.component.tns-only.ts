...
export class NativeScriptLineChartExampleComponent implements OnInit {
  ...
  mapToChartItems(
    data: { rate: number; indexRate: number; rateDate: string; rateTime: string; sales: number }[]
  ): Observable<NativeScriptLineChartItem[]> {
    ...
    return distributions$.pipe(
      map((distributions) => {
        return distributions.map((distribution) => {
          ...
          return new NativeScriptLineChartItem(date, distribution.rate);
        });
      })
    );
  }
