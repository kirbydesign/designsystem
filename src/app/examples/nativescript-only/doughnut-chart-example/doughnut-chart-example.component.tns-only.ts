...
export class NativeScriptDoughnutChartExampleComponent implements OnInit {
  ...
  ngOnInit() {
    this.chartItems$ = this.mapToChartItems(this.getMockData(), this._paletteColors).pipe(
      delay(500)
    );
  }
  ...
  mapToChartItems(
    data: { type: string; labelPercentage: string; drawingPercentage: number }[],
    colors: string[]
  ): Observable<NativeScriptChartItem[]> {
    let index = 0;
    const distributions$ = of(data);
    return distributions$.pipe(
      map((distributions) => {
